...
/** Tipo da categoria de força da mão: [tipo, nível] */
export type HandStrengthCategory = readonly [string, string];

/** Mão com categoria de força já classificada (para one-hot). */
export type ClassifiedHand = PokerHand & { hand_strength: HandStrengthCategory };
...
export class TexasHoldem {
     ...
     * Cada sub-array contém dois textos: o tipo da mão (como 'pair', 'suitted' ou 'off suitted'),
     * e se ela é considerada 'strong' (forte) ou 'weak' (fraca).
     * Exemplo: ['pair', 'strong'] refere-se a pares de mão inicial fortes (AA, KK, QQ...).
     */
    static readonly hand_strength = [
        ['pair', 'strong'],        // Ex: AA, KK, QQ, JJ
        ['pair', 'weak'],          // Ex: 22, 33, 44, pares baixos
        ['suitted', 'strong'],     // Duas cartas do mesmo naipe, valores altos (Ex: AKs, QJs)
        ['suitted', 'weak'],       // Duas cartas do mesmo naipe, valores baixos/desconectados
        ['off suitted', 'strong'], // Duas cartas de naipes diferentes, valores altos (Ex: AKo)
        ['off suitted', 'weak']    // Duas cartas de naipes diferentes, valores baixos
    ]
     ...
     * Exemplo: uma mão 'suitted strong' seria [0, 0, 1, 0, 0, 0].
     */
    static readonly hand_strength_to_one_hot = [
        [1, 0, 0, 0, 0, 0], // 'pair strong'
        [0, 1, 0, 0, 0, 0], // 'pair weak'
        [0, 0, 1, 0, 0, 0], // 'suitted strong'
        [0, 0, 0, 1, 0, 0], // 'suitted weak'
        [0, 0, 0, 0, 1, 0], // 'off suitted strong'
        [0, 0, 0, 0, 0, 1]  // 'off suitted weak'
    ]

    static handStrengthToOneHot(hand: ClassifiedHand): number[] {
        const idx = TexasHoldem.hand_strength.findIndex(
            (row) => row[0] === hand.hand_strength[0] && row[1] === hand.hand_strength[1]
        );
        if (idx === -1) {
            throw new Error(`Unknown hand_strength: [${hand.hand_strength[0]}, ${hand.hand_strength[1]}]`);
        }
    ...
    static classifyHand(hand: PokerHand): HandStrengthCategory {
        ...
        const hasHigh = first.value >= STRONG_THRESHOLD || second.value >= STRONG_THRESHOLD;
        const level = hasHigh ? 'strong' : 'weak';
        if (isPair) return ['pair', level];
        if (isSuited) return ['suitted', level];
        return ['off suitted', level];
    }
}